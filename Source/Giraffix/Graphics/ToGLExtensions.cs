using System;
using Giraffix.Native.OpenGL;

namespace Giraffix.Graphics;

public static class ToGLExtensions
{
    public static GL.MapFlags ToGL(this GL.MapFlags mapFlags)
    {
        var result = GL.MapFlags.Read;
        if ((mapFlags & GL.MapFlags.Read) == GL.MapFlags.Read)
        {
            result |= GL.MapFlags.Read;
        }

        if ((mapFlags & GL.MapFlags.Write) == GL.MapFlags.Write)
        {
            result |= GL.MapFlags.Write;
        }

        if ((mapFlags & GL.MapFlags.Persistent) == GL.MapFlags.Persistent)
        {
            result |= GL.MapFlags.Persistent;
        }

        if ((mapFlags & GL.MapFlags.Coherent) == GL.MapFlags.Coherent)
        {
    ...
    public static GL.BlitFramebufferFilter ToGL(this GL.BlitFramebufferFilter blitFramebufferFilter)
    {
        return blitFramebufferFilter switch
        {
            GL.BlitFramebufferFilter.Linear => GL.BlitFramebufferFilter.Linear,
            GL.BlitFramebufferFilter.Nearest => GL.BlitFramebufferFilter.Nearest,
            _ => throw new ArgumentOutOfRangeException(nameof(blitFramebufferFilter))
        };
    }

    public static GL.FramebufferBit ToGL(this GL.FramebufferBit framebufferBit)
    {
        GL.FramebufferBit result = 0u;
        result |= (framebufferBit & GL.FramebufferBit.ColorBufferBit) == GL.FramebufferBit.ColorBufferBit
            ? GL.FramebufferBit.ColorBufferBit
            : 0;
        result |= (framebufferBit & GL.FramebufferBit.DepthBufferBit) == GL.FramebufferBit.DepthBufferBit
            ? GL.FramebufferBit.DepthBufferBit
            : 0;
        result |= (framebufferBit & GL.FramebufferBit.StencilBufferBit) == GL.FramebufferBit.StencilBufferBit
